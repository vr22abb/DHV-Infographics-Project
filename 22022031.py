...
norm = plt.Normalize(vmin=pollution_data['mostPollutedCountries_particlePollution'].min(),
                     vmax=pollution_data['mostPollutedCountries_particlePollution'].max())
...
fig, axs = plt.subplots(2, 2, figsize=(18, 12), gridspec_kw={'height_ratios': [2, 2], 'width_ratios': [2, 2],'hspace': 0.7})
plt.subplots_adjust(left=0.2, bottom=0.1, right=0.9, top=0.85, wspace=0.4, hspace=0.4)
...
def plot_particle_pollution(data, specific_countries, scalar_map, ax):
    ...
    data_specific_sorted = data_specific.sort_values(by='mostPollutedCountries_particlePollution', ascending=False)    
    pollution_values = data_specific_sorted['mostPollutedCountries_particlePollution']
    bar_colors = scalar_map.to_rgba(pollution_values)  
    bars = ax.barh(data_specific_sorted['country_name'], pollution_values, color=bar_colors)
    ax.set_xlabel('Particle Pollution (µg/m³)', fontsize=12, labelpad=40)
    ax.set_ylabel('Country', fontsize=12)
    ax.set_title('Particle Pollution for Specified Countries', fontsize=13, pad=7)
    ...
    ax.grid(True, which='both', axis='both', color='gray', linestyle='-', linewidth=0.2)  
...
def plot_pollution_growth_and_density(data, ax1):
    ...
    """
    avg_pollution_growth_by_region = data.groupby('country_region')['pollution_growth_Rate'].mean()
    avg_pollution_density_by_region = data.groupby('country_region')['pollution_density_per_Mile'].mean()
    sorted_regions = avg_pollution_growth_by_region.sort_values().index
    ax1.fill_between(sorted_regions, avg_pollution_density_by_region.reindex(sorted_regions), color='darkgreen', alpha=0.5, label='Pollution Density per Mile')
    ax2 = ax1.twinx()
    ax2.plot(sorted_regions, avg_pollution_growth_by_region.reindex(sorted_regions), marker='o', linestyle='-', color='red', label='Pollution Growth Rate')
    ax1.set_xlabel('Region', fontsize=12, labelpad=2)
    ax1.set_ylabel('Average Pollution Density per Mile', fontsize=12, color='darkgreen')
    ...
    ax1.set_title('Average Pollution Growth Rate and Density per Mile by Region', fontsize=13, pad=7)
    ax1.grid(True, which='both', axis='both', color='gray', linestyle='-', linewidth=0.2)
...
def plot_pollution_across_regions(data, ax):
    ...
    region_pollution_percent = (region_pollution / region_pollution.sum()) * 100
    explode = [0.04] * len(region_pollution_percent)
    wedges, texts, autotexts = ax.pie(region_pollution_percent, autopct='%1.1f%%', startangle=200,
                                      colors=plt.cm.RdYlGn(np.linspace(0, 1, len(region_pollution_percent))),
                                      explode=explode, wedgeprops=dict(width=0.7), pctdistance=1.2)
    ax.set_title('Percentage of Pollution produced across Regions', fontsize=15, pad=10)
    ax.text(0, 0, '2023', ha='center', va='center', size=10, color="black", weight='bold')
    for text in texts:
        text.set_fontsize(10)  
    legend = ax.legend(wedges, region_pollution_percent.index, title="Region", loc="center left",
...
def plot_comparative_regional_analysis(data, ax):
    ...
    bars = ax.bar(region_grouped['country_region'], region_grouped['country_land_Area_in_Km'], color=colors)
    ax.set_xlabel('Region', fontsize=12, labelpad=1)
    ax.set_ylabel('Average Land Area in Km', color='tab:blue', fontsize=12)
    ax.tick_params(axis='x', rotation=90, labelsize=10)  
    ax.tick_params(axis='y', labelsize=10)  
    ax.grid(True, which='both', axis='y', color='gray', linestyle='-', linewidth=0.1)
    ax2 = ax.twinx()
    ax2.plot(region_grouped['country_region'], region_grouped['pollution_density_in_km'], color='red', marker='o')
    ax2.set_ylabel('Average Pollution Density (per Km)', color='tab:red', fontsize=12)
    ax2.tick_params(axis='y', labelsize=10)  # Increase font size for y-axis tick labels
    ax2.grid(True, which='both', axis='both', color='gray', linestyle='-', linewidth=0.4)
...
specific_countries = ['India', 'China', 'United States', 'Australia', 'Japan', 'Iceland', 'Pakistan']
plot_particle_pollution(pollution_data, specific_countries, scalar_map, axs[0, 0])
...
plt.suptitle('Global Pollution Analysis - 2023', fontsize=20, weight='bold', ha='center')
...
plt.figtext(0.2, -0.3, Plot_description, ha='left', va='center', fontsize=15, wrap=True, weight='bold')

# Add highlighted name and student ID
student_details = "Name       : Vinoth Rajendran\nStudent ID : 22022031"
plt.figtext(0.2, -0.01, student_details, ha='left', va='center', fontsize=12, weight='bold', bbox=student_details_params)
